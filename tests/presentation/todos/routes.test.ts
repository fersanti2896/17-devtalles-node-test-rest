...
describe('Test in the routes.ts', () => { 
    ...
    test('should return TODOs api/todos.', async() => { 
        ...
        const respose = await request( testServer.app ).get( '/api/todos' ).expect( 200 );
    ...
    test('should return a new Todo api/todos', async() => { 
        const { body } =  await request( testServer.app ).post( '/api/todos' )
                                                         .send( todo1 )
                                                         .expect( 201 );
    ...
    test('should return a updateTodo TODO api/todos/:id', async() => { 
        const todo = await prisma.todo.create({ data: todo1 });

        const { body } =  await request( testServer.app ).put( `/api/todos/${ todo.id }` )
                                                         .send({ text: 'Hola mundo update', completedAt: '2024-01-17' })
                                                         .expect( 200 );

        expect( body ).toEqual({
            id: expect.any(Number),
            text: 'Hola mundo update',
            completedAt: '2024-01-17T00:00:00.000Z'
        });
    });
    ...
    test('should return a 404 if TODO not found api/todos/:id', async() => { 
        ...
        const { body } =  await request( testServer.app ).put( `/api/todos/${ todoId }` )
                                                         .send({ text: 'Hola mundo update', completedAt: '2024-01-17' })
                                                         .expect( 400 );
    ...
    test('should return an updated TODO only the text api/todos/:id', async() => { 
        ...
        const { body } =  await request( testServer.app ).put( `/api/todos/${ todo.id }` )
                                                         .send({ text: 'Hola mundo actualizado' })
                                                         .expect( 200 );
    ...
    test('should return an updated TODO only the id api/todos/:id', async() => { 
        const todo = await prisma.todo.create({ data: todo1 });
    ...
    test('should delete a TODO api/todos/:id', async() => { 
        ...
        const { body } =  await request( testServer.app ).delete( `/api/todos/${ todo.id }` )
                                                         .expect( 200 );
    ...
    test('should return 404 if TODO do not exist api/todos/:id', async() => { 
        const todoId = 999;
        const { body } =  await request( testServer.app ).delete( `/api/todos/${ todoId }` )
                                                         .expect( 400 );
